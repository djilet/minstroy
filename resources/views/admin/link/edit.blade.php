...
@section('title')Редактировать ссылку@endsection


@section('content')
    <section class="box">
        <header class="panel_header">
            <h2 class="title pull-left">Редактировать ссылку</h2>
        </header>
        ...
        <div class="content-body">
            ...
            <form action="{{ route('admin.link.update', $page->id) }}" method="post" name="page-form" id="page-form" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="checkbox" name="active" id="Active" value="1" class="iswitch iswitch-md iswitch-primary"{{ old('active', $page->active) == 1 ? ' checked' : '' }} />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="type" id="type" onchange="ChangeLinkType(this);" class="form-control">
                                        <option value="url"{{ $page->link->type == 'url' ? ' selected' : '' }}>URL</option>
                                        <option value="email"{{ $page->link->type == 'email' ? ' selected' : '' }}>Эл. почта</option>
                                        <option value="internal"{{ $page->link->type == 'internal' ? ' selected' : '' }}>Страница вашего сайта</option>
                                    </select>
                                </div>
                                ...
                                <div id="divLinkTypeInternalPage" style="display:none;" class="form-group">
                                    ...
                                    <select id="cmbLinkInternalPage" class="form-control">
                                        <option>...</option>
                                        @foreach($pages as $p)
                                            <option value="{{ $p->full_slug }}"{{ $p->full_slug == $page->link->link_url ? ' selected' : '' }}>{{ $p->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
...
@push('footer.scripts')
    ...
    <script>
        ...
        var sType = FillLinkDivFromHRef(
            "{{ old('link_url', $page->link->link_url) }}", 
            "{{ old('target', $page->link->target) }}",
            "{{ old('type', $page->link->type) }}"
        );
