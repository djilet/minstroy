...
function FillLinkDivFromHRef(sHRef, sTarget, type)
{
	var sType = "url";
	...
	internalPageFound = (type === 'internal');
...
function GenerateLinkHRef(form)
{
	...
	switch (form.elements["type"].options[form.elements["type"].selectedIndex].value)
	{
		case "url":
			sUri = $("#txtUrl").val();
			if (sUri.length == 0)
